...
.search-hero .hero-section {
  ...
  background-image: linear-gradient(130deg, #04c 0%, #ff5200 89%);
  ...
  <div class="container {{ $posts->isNotEmpty() ? 'tw-mt-[-130px]' : '' }}">
    <div class="{{ $gridClass }}">
      ...
      <div class="tw-col-span-full tw-text-center">
        <p class="tw-text-black tw-font-bold tw-text-xl tw-mb-5">
          {{ __("It seems we can't find what you're looking for") }}
        </p>

        <div>
          @if(theme_option('search_no_result_image'))
          {{ RvMedia::image(theme_option('search_no_result_image'), __('No search results'), 'high', attributes: ['class' => 'tw-mx-auto tw-max-w-96']) }}
          @else
