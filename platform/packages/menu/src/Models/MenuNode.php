...
class MenuNode extends BaseModel
{
    ...
    protected function url(): Attribute
    {
        return Attribute::get(function ($value) {
            ...
            // If this is a custom link (no reference), return the stored value.
            if (! $this->reference_type) {
                return $value;
            }

            // If reference type is set but the referenced model can't be loaded,
            // fall back to the stored value.
            if (! $this->reference) {
                return $value;
            }

            // For referenced menu items, always use the referenced model URL.
            // This keeps menus in sync with permalink prefix changes automatically.
            $url = (string) $this->reference->url;

            return apply_filters(MENU_FILTER_NODE_URL, $url ?: $value);
        });
    }
