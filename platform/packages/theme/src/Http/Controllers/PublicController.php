...
class PublicController extends BaseController
{
    ...
    public function getView(?string $key = null, string $prefix = '')
    {
        ...
        if (! $slug && ! empty($prefix)) {
            $slug = SlugHelper::getSlug($key);

            if ($slug) {
                $expectedPrefix = (string) SlugHelper::getPrefix($slug->reference_type, '', false);

                if ($expectedPrefix !== '' && $slug->prefix !== $expectedPrefix) {
                    $slug->forceFill(['prefix' => $expectedPrefix])->saveQuietly();

                    try {
                        DB::table('slugs_translations')
                            ->where('slugs_id', $slug->getKey())
                            ->update(['prefix' => $expectedPrefix]);
                    } catch (Throwable) {

                    }
                }

                $slug = SlugHelper::getSlug($key, $prefix) ?: $slug;
            }
        }
