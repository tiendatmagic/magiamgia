    ...
    @else
        ...
        @if (Auth::user() && $id && is_in_admin(true))
            @php
                ...
                $showHtmlExtension = ! in_array($modelClass, [
                    'Botble\Page\Models\Page',
                    'Botble\Blog\Models\Category',
                    'Botble\Service\Models\Category',
                    'Botble\Blog\Models\Tag',
                ]);
                $displayURL = $showHtmlExtension ? $previewURL . '.html' : $previewURL;
            @endphp
            <x-core::form.helper-text class="mt-n2 text-truncate">
                {{ trans('packages/slug::slug.preview') }}: <a href="{{ $displayURL }}" target="_blank">{{ $displayURL }}</a>
