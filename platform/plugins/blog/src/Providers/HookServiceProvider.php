...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        FormAbstract::beforeRendering(function (FormAbstract $form) {
            ...
            // Language Advanced (v2) removes non-translatable fields in translation mode.
            // Categories/Tags are shared across languages (same post record) but still useful
            // to edit while viewing the translated content, so we re-add them.
            ...
            if (! array_key_exists('categories[]', $fields)) {
                $form->add(
                    ...
                    SelectFieldOption::make()
                        ...
                        ->when($model->getKey(), function (SelectFieldOption $fieldOption) use ($model) {
                            return $fieldOption->selected($model->categories()->pluck('category_id')->all());
                        })
                        ->when(! $model->getKey(), function (SelectFieldOption $fieldOption) {
                            return $fieldOption
            ...
            if (! array_key_exists('tag', $fields)) {
                $form->add(
                    ...
                    TagFieldOption::make()
                        ...
                        ->when($model->getKey(), function (TagFieldOption $fieldOption) use ($model) {
                            return $fieldOption
                                ->selected(
                                    $model
                                        ->tags()
        ...
        if (defined('LANGUAGE_ADVANCED_ACTION_SAVED')) {
            add_action(LANGUAGE_ADVANCED_ACTION_SAVED, function ($data, $request) {
                if (! $data instanceof Post || ! $request) {
                    return;
                }

                // Persist shared relationships when saving a translation.
                if ($request->has('categories')) {
                    app(StoreCategoryService::class)->execute($request, $data);
                }

                if ($request->has('tag')) {
                    app(StoreTagService::class)->execute($request, $data);
                }
            }, 1200, 2);
        }
